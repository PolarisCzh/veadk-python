...
### Per-creation MPA YAML / 单次创建上传配置

Studio administrators may select a UTF-8 `.yaml` or `.yml` file (at most 256 KiB) in the MPA creation dialog. The file is validated by the same managed-profile contract as `VEADK_MPA_CREATE_CONFIG` and is used only for that creation task; it does not replace the server's default YAML. An uploaded profile cannot reference `managed.template-file` or `managed.credential-file`. The Studio server must still have deployment credentials and the PostgreSQL connection environment variables referenced by the profile. The private temporary file is tied to the task and removed on completion, cancellation, or rejected submission; only its digest is retained for request-idempotency checks. The browser does not persist the YAML contents, so a retry after reopening the dialog requires reselecting the same file. Upload is available only to Studio administrators; other users retain the server-configured creation flow.

Studio 管理员可以在创建 MPA 弹窗选择不超过 256 KiB 的 UTF-8 `.yaml` / `.yml` 文件。服务端按 `VEADK_MPA_CREATE_CONFIG` 相同的托管配置契约校验，仅用于本次创建任务，不修改服务端默认 YAML。上传配置不允许引用 `managed.template-file` 或 `managed.credential-file`；Studio 服务端仍需具备部署凭据和配置所引用的 PostgreSQL 连接环境变量。私有临时文件与任务绑定，在完成、取消或拒绝提交后删除；数据库仅保存摘要用于同一请求的幂等校验。浏览器不保存 YAML 正文，因此重新打开弹窗后重试需要重新选择相同文件。仅 Studio 管理员可上传，其他用户仍使用服务端配置创建。
